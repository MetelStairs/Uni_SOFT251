...
public class LoginController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String page = "/WEB-INF/SecretaryViews/login.jsp";
        ...
        request.setAttribute("secretary", user);
        if (user != null) {
            if (user.getType().equals("secretary")) {
                request.getSession().setAttribute("secretary", user);
                response.sendRedirect(response.encodeRedirectURL(request.getContextPath() + "/Secretary"));
